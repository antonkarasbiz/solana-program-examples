...
///   account-type byte (1)                                  +
///   TLV entry: type (2) + length (2) + value (1)           = 171
/// ```
///
/// The base is padded up to a token *Account*'s length (165) so a mint and an
/// account can never be the same size, and the `DefaultAccountState` value is a
/// single `AccountState` byte. This mirrors
/// `ExtensionType::try_calculate_account_len::<Mint>(&[DefaultAccountState])`.
pub const MINT_SIZE: usize = 171;
