...
const PROGRAM_SO = path.join(process.cwd(), 'tests', 'fixtures', 'token_2022_permanent_delegate_pinocchio_program.so');

describe('Token-2022 Permanent Delegate (Pinocchio)', () => {
    let svm: LiteSVM;
    ...
    it('Creates a Token-2022 mint with a permanent delegate', async () => {
        ...
        // A distinct key for the permanent delegate so the stored-delegate
        // assertion verifies it is sourced from account index 2, not the mint
        // authority/payer.
        ...
        const ix = {
            ...
            accounts: [
                ...
                { address: permanentDelegate.address, role: AccountRole.READONLY }, // permanent delegate
        ...
        const permanentDelegateExtension = extensions.find(e => e.__kind === 'PermanentDelegate');
        if (permanentDelegateExtension?.__kind !== 'PermanentDelegate') {
            throw new Error('PermanentDelegate extension not found on the mint');
        }

        // The configured permanent delegate was stored in the extension.
        assert.equal(permanentDelegateExtension.delegate, permanentDelegate.address);
