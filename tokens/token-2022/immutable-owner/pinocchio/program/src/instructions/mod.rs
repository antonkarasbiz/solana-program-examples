...
///   account-type byte (1)                              +
///   ImmutableOwner TLV: type (2) + length (2) + value (0) = 170
/// ```
///
/// `ImmutableOwner` stores no value, so its TLV entry is just the 4-byte header.
/// This mirrors
/// `ExtensionType::try_calculate_account_len::<Account>(&[ImmutableOwner])`.
pub const ACCOUNT_SIZE: usize = 170;
