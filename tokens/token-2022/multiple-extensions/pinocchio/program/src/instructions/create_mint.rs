...
/// Creates a new SPL Token-2022 mint that carries *two* extensions at once:
/// `MintCloseAuthority` (lets the configured authority later close the mint and
/// reclaim its rent) and `NonTransferable` (tokens minted from it can never be
/// moved — every transfer, outside of burning or closing the account, is
/// rejected by the Token-2022 program).
///
/// Both extensions must be initialized *before* `InitializeMint`: extensions live
/// in the space past the base mint, and Token-2022 rejects initializing them once
/// the mint itself has been initialized. The order of the two extension
/// initializations relative to each other does not matter.
///
/// Accounts:
...
pub fn create_mint(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    ...
    // Both extension init instructions take just the mint account.
    let extension_accounts = [InstructionAccount::writable(mint_account.address())];
