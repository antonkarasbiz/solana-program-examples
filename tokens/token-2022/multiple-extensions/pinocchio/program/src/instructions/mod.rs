...
/// Size (in bytes) of a Token-2022 mint account that carries *both* the
...
///   account-type byte (1)                                       +
///   MintCloseAuthority TLV: type (2) + length (2) + value (32)  +
///   NonTransferable TLV:    type (2) + length (2) + value (0)   = 206
/// ```
///
/// The base is padded up to a token *Account*'s length (165) so a mint and an
/// account can never be the same size. `MintCloseAuthority` stores a single
/// optional pubkey (32 bytes); `NonTransferable` stores no value, so its TLV
/// entry is just the 4-byte header. This mirrors
/// `ExtensionType::try_calculate_account_len::<Mint>(&[MintCloseAuthority, NonTransferable])`.
pub const MINT_SIZE: usize = 206;
