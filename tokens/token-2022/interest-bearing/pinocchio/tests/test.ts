...
//   base account length (165) + account-type byte (1) + TLV entry (56) = 222
// The InterestBearingConfig TLV entry is type (2) + length (2) + value (52).
const EXTENDED_MINT_SIZE = 222;
...
const PROGRAM_SO = path.join(process.cwd(), 'tests', 'fixtures', 'token_2022_interest_bearing_pinocchio_program.so');

describe('Token-2022 Interest Bearing (Pinocchio)', () => {
    let svm: LiteSVM;
    ...
    it('Creates a Token-2022 mint with an interest bearing config', async () => {
        // Matches the anchor reference example's mint decimals.
        const decimals = 2;
        ...
        const ix = {
            ...
            accounts: [
                ...
                // The mint authority doubles as the freeze authority. It is the same
                // key as the payer, which is also the rate authority and signs the
                // post-init rate update — so one signature covers all of it.
        ...
        const interestBearingExtension = extensions.find(e => e.__kind === 'InterestBearingConfig');
        if (interestBearingExtension?.__kind !== 'InterestBearingConfig') {
            throw new Error('InterestBearingConfig extension not found on the mint');
        }

        // The payer was stored as the rate authority.
        assert.equal(interestBearingExtension.rateAuthority, payer.address);

        // The program initializes the rate to 0%, then updates it to 1% (100 bp) —
        // so the current rate is the 1% one.
        assert.equal(interestBearingExtension.currentRate, 100);
