...
///   account-type byte (1)                                  +
///   TLV entry: type (2) + length (2) + value (0)           = 170
/// ```
///
/// The base is padded up to a token *Account*'s length (165) so a mint and an
/// account can never be the same size. Unlike `MintCloseAuthority`, the
/// `NonTransferable` extension stores no value, so its TLV entry is just the
/// 4-byte header. This mirrors
/// `ExtensionType::try_calculate_account_len::<Mint>(&[NonTransferable])`.
pub const MINT_SIZE: usize = 170;
